...
const MoveOrganizationModal: React.FC<MoveOrganizationModalProps> = ({
  ...
  const handleParentChange = (value: string) => {
    ...
    if (parent?.children) {
      formRef.current?.setFieldsValue({ newSortOrder: parent.children.length });
    } else {
      formRef.current?.setFieldsValue({ newSortOrder: 0 });
    }
  };

  // 处理表单提交
  const handleSubmit = async (values: { newParentId?: string; newSortOrder?: number }) => {
