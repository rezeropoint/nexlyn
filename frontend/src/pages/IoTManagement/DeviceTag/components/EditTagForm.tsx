import type { UpdateTagRequest } from "@/services/iot";
import {
  DrawerForm,
  ProFormText,
  ProFormTextArea,
...
import styles from "./EditTagForm.less";

interface EditTagFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onFinish: (values: UpdateTagRequest) => Promise<boolean>;
  currentRow?: DeviceTagSummary;
}
...
const EditTagForm: React.FC<EditTagFormProps> = ({
  ...
  useEffect(() => {
    if (currentRow && open) {
      ...
      setSelectedColor(currentRow.color || DEFAULT_COLOR);
    }
  }, [currentRow, open]); // 依赖项不包含 formRef

  return (
    <DrawerForm<UpdateTagRequest>
      formRef={formRef}
      title="编辑标签"
      open={open}
