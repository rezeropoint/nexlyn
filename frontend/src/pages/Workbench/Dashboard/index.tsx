...
const Dashboard: React.FC = () => {
  ...
  const [pendingStats, setPendingStats] =
    useState<GetPendingStatsResponse['data']>();
  const [durationStats, setDurationStats] =
    useState<GetDurationStatsResponse['data']>();
  const [statusStats, setStatusStats] =
    useState<GetStatusStatsResponse['data']>();
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      ...
      try {
        ...
        const [pendingRes, durationRes, statusRes, trendRes, userRes] =
          await Promise.all([
        ...
        if (pendingRes.code === 0) {
          ...
          message.warning(
            `待处理统计加载失败: ${getErrorMessage(pendingRes, '未知错误')}`,
          );
        }
        ...
        if (durationRes.code === 0) {
          ...
          message.warning(
            `时长统计加载失败: ${getErrorMessage(durationRes, '未知错误')}`,
          );
        }
        ...
        if (statusRes.code === 0) {
          ...
          message.warning(
            `状态统计加载失败: ${getErrorMessage(statusRes, '未知错误')}`,
          );
        }
        ...
        if (trendRes.code === 0) {
          ...
          message.warning(
            `趋势统计加载失败: ${getErrorMessage(trendRes, '未知错误')}`,
          );
        }
        ...
        if (userRes.code === 0) {
          ...
          message.warning(
            `处理人统计加载失败: ${getErrorMessage(userRes, '未知错误')}`,
          );
        }
      } catch (error: any) {
  ...
  const handleViewDetail = (task: Assignment) => {
    ...
    history.push(
      `/workbench/pending-tasks?flowId=${task.flowId}&journeyId=${task.journeyId}&assignmentId=${task.id}`,
    );
  };
  ...
  const heroMetrics = [
    ...
    },
    {
      label: '处理中',
      value:
        statusStats?.statusCounts?.find(
          (item) => item.statusKey === 'processing',
        )?.count ?? 0,
    ...
    },
    {
      label: '已完成',
      value:
        statusStats?.statusCounts?.find((item) => item.statusKey === 'finished')
          ?.count ??
        ...
        <div className={styles.leftColumn}>
          ...
          <div className={styles.analyticsRow}>
            <div className={styles.wideCard}>
              <DeviceOverview
                orgId={currentUser?.organizationIds?.[0]}
        ...
        <div className={styles.rightColumn}>
          <Card
            ...
            title={
              <span>
                <ClockCircleOutlined
                  style={{ marginRight: 8, color: 'var(--ant-color-warning)' }}
                />
                待办任务
              </span>
            }
