...
func (l *GetDeviceStatisticsLogic) GetDeviceStatistics(req *types.GetDeviceStatisticsRequest) (resp *types.GetDeviceStatisticsResponse, err error) {
	// 记录操作开始
	...
	if err != nil {
		...
		return &types.GetDeviceStatisticsResponse{
	...
	// 5. 构建core.DeviceStatisticsQuery
	query := core.DeviceStatisticsQuery{
	...
	// 6. 调用IoT引擎获取设备统计信息
	result, err := l.svcCtx.IoTEngine.GetDeviceStatistics(l.ctx, query)
	if err != nil {
	...
	// 7. 转换结果为API类型
	data := svc.ConvertCoreDeviceStatisticsToTypes(result)
