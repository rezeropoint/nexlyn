...
func (l *GetUserStatsLogic) GetUserStats(req *types.GetUserStatsRequest) (resp *types.GetUserStatsResponse, err error) {
	// 记录操作开始
	...
	// 调用Skylark引擎查询处理人统计
	statsResp, err := l.svcCtx.SkylarkEngine.GetUserStats(l.ctx, statsReq)
	if err != nil {
	...
	// 转换响应格式
	response := svc.ConvertCoreUserStatsToTypes(statsResp)
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("event_ids", req.EventIds),
		logx.Field("user_count", len(statsResp.UserMetrics)),
