...
// 获取节点统计
func NewGetNodeStatsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetNodeStatsLogic {
	return &GetNodeStatsLogic{
...
func (l *GetNodeStatsLogic) GetNodeStats(req *types.GetNodeStatsRequest) (resp *types.GetNodeStatsResponse, err error) {
	// 记录操作开始
	...
	// 调用Skylark引擎查询节点统计
	statsResp, err := l.svcCtx.SkylarkEngine.GetNodeStats(l.ctx, statsReq)
	if err != nil {
	...
	// 转换响应格式
	response := svc.ConvertCoreNodeStatsToTypes(statsResp)
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("event_ids", req.EventIds),
		logx.Field("node_count", len(statsResp.NodeMetrics)),
	).Info("获取节点统计成功")
