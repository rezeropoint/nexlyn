...
type GetTrendStatsLogic struct {
	logx.Logger
	ctx    context.Context
...
// 获取趋势统计
func NewGetTrendStatsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetTrendStatsLogic {
	return &GetTrendStatsLogic{
...
func (l *GetTrendStatsLogic) GetTrendStats(req *types.GetTrendStatsRequest) (resp *types.GetTrendStatsResponse, err error) {
	// 记录操作开始
	...
	if groupBy != "day" && groupBy != "week" && groupBy != "month" {
		...
		return &types.GetTrendStatsResponse{
			BaseResponse: types.BaseResponse{Code: 400, Msg: "GroupBy参数无效，必须是day/week/month之一"},
		}, nil
	}
	...
	// 调用Skylark引擎查询趋势统计
	statsResp, err := l.svcCtx.SkylarkEngine.GetTrendStats(l.ctx, statsReq)
	if err != nil {
	...
	// 转换响应格式
	response := svc.ConvertCoreTrendStatsToTypes(statsResp)
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("event_ids", req.EventIds),
		logx.Field("time_point_count", len(statsResp.TimePoints)),
	).Info("获取趋势统计成功")
