...
type GetPendingStatsLogic struct {
	logx.Logger
	ctx    context.Context
...
// 获取待处理事件统计
func NewGetPendingStatsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetPendingStatsLogic {
	return &GetPendingStatsLogic{
...
func (l *GetPendingStatsLogic) GetPendingStats(req *types.GetPendingStatsRequest) (resp *types.GetPendingStatsResponse, err error) {
	// 记录操作开始
	...
	// 调用Skylark引擎查询待处理事件统计（实时查询，不缓存）
	statsResp, err := l.svcCtx.SkylarkEngine.GetPendingStats(l.ctx, statsReq)
	if err != nil {
	...
	// 转换响应格式
	response := svc.ConvertCorePendingStatsToTypes(statsResp)
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("event_ids", req.EventIds),
		logx.Field("pending_count", statsResp.TotalPending),
		logx.Field("processing_count", statsResp.TotalProcessing),
		logx.Field("total", statsResp.Total),
	).Info("获取待处理事件统计成功")
