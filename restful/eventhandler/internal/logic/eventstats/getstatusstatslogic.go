...
type GetStatusStatsLogic struct {
	logx.Logger
	ctx    context.Context
...
// 获取状态统计
func NewGetStatusStatsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetStatusStatsLogic {
	return &GetStatusStatsLogic{
...
func (l *GetStatusStatsLogic) GetStatusStats(req *types.GetStatusStatsRequest) (resp *types.GetStatusStatsResponse, err error) {
	// 记录操作开始
	...
	// 调用Skylark引擎查询状态统计
	statsResp, err := l.svcCtx.SkylarkEngine.GetStatusStats(l.ctx, statsReq)
	if err != nil {
	...
	// 转换响应格式
	response := svc.ConvertCoreStatusStatsToTypes(statsResp)
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("event_ids", req.EventIds),
		logx.Field("status_count", len(statsResp.StatusCounts)),
