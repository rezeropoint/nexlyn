...
type GetJourneyMomentsLogic struct {
	logx.Logger
	ctx    context.Context
...
// 获取审批历史时间线
func NewGetJourneyMomentsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetJourneyMomentsLogic {
	return &GetJourneyMomentsLogic{
...
func (l *GetJourneyMomentsLogic) GetJourneyMoments(req *types.GetJourneyMomentsRequest) (resp *types.GetJourneyMomentsResponse, err error) {
	// 记录操作开始
	logx.WithContext(l.ctx).WithFields(
		logx.Field("service", l.svcCtx.Config.RestConf.Name),
		logx.Field("pod", l.svcCtx.PodName),
		logx.Field("module", "flow_journey"),
		logx.Field("operation", "get_journey_moments"),
	...
	if err != nil {
		...
		return &types.GetJourneyMomentsResponse{
	...
	// 3. 调用Skylark引擎获取审批历史
	moments, err := l.svcCtx.SkylarkEngine.GetJourneyMoments(
		l.ctx,
		jwtUser.TenantId,
		req.JourneyId,
	)
	...
	// 4. 转换结果
	data := svc.ConvertCoreMomentListToTypes(moments)
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("tenant_id", jwtUser.TenantId),
		logx.Field("journey_id", req.JourneyId),
		logx.Field("moments_count", len(data)),
	).Info("获取审批历史成功")
