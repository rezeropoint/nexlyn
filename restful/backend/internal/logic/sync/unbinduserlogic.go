...
func (l *UnbindUserLogic) UnbindUser(req *types.UnbindUserRequest) (resp *types.UnbindUserResponse, err error) {
	// 1. 检查是否启用Skylark同步
	if !l.svcCtx.Config.SkylarkSyncEnabled || l.svcCtx.AdminSyncClient == nil {
		return &types.UnbindUserResponse{
	...
	// 4. 调用 eventsync gRPC 解绑用户
	unbindResp, err := l.svcCtx.AdminSyncClient.UnbindUser(l.ctx, &pb.UnbindUserReq{
		TenantId:    currentUser.TenantId,
		LocalUserId: req.Id,
	})

	if err != nil {
		logx.Error("调用eventsync解绑用户失败: ", err)
		return &types.UnbindUserResponse{
			BaseResponse: types.BaseResponse{
				Code: 500,
