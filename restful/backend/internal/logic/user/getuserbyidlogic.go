...
func (l *GetUserByIdLogic) GetUserById(req *types.GetUserByIdRequest) (resp *types.GetUserByIdResponse, err error) {
	...
	logx.WithContext(l.ctx).WithFields(
		...
		logx.Field("operation", "get_user_by_id"),
	...
	if err != nil {
		return &types.GetUserByIdResponse{BaseResponse: types.BaseResponse{Code: 401, Msg: "用户认证失败"}}, nil
	}
	...
	err = l.svcCtx.DBConn.QueryRow(&user, query, req.Id)
	if err != nil {
		logx.WithContext(l.ctx).WithFields(
			logx.Field("service", l.svcCtx.Config.RestConf.Name),
			logx.Field("pod", l.svcCtx.PodName),
	...
	// 检查跨租户权限
	isCrossTenant := user.TenantIdStr != "" && user.TenantIdStr != jwtUser.TenantId
	if isCrossTenant {
		...
		if !hasCrossTenantPermission {
			return &types.GetUserByIdResponse{
				BaseResponse: types.BaseResponse{Code: 403, Msg: "权限不足：需要跨租户用户查看权限"},
			}, nil
		}
	}
	...
	currentUser := types.User{
		...
		Title:           user.Title,
		TenantInfo:      types.TenantInfo{TenantId: user.TenantIdStr, TenantName: user.TenantName},
