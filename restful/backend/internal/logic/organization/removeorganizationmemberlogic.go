...
type RemoveOrganizationMemberLogic struct {
	logx.Logger
	ctx    context.Context
...
// 移除组织成员
func NewRemoveOrganizationMemberLogic(ctx context.Context, svcCtx *svc.ServiceContext) *RemoveOrganizationMemberLogic {
	return &RemoveOrganizationMemberLogic{
...
func (l *RemoveOrganizationMemberLogic) RemoveOrganizationMember(req *types.RemoveOrganizationMemberRequest) (resp *types.RemoveOrganizationMemberResponse, err error) {
	// 记录操作开始
	logx.WithContext(l.ctx).WithFields(
		logx.Field("service", l.svcCtx.Config.RestConf.Name),
		logx.Field("pod", l.svcCtx.PodName),
		logx.Field("module", "organization"),
		logx.Field("operation", "remove_organization_member"),
	...
	if err != nil {
		...
		return &types.RemoveOrganizationMemberResponse{
	...
	err = l.svcCtx.DBConn.TransactCtx(l.ctx, func(ctx context.Context, session sqlx.Session) error {
		...
		removeQuery := `
			...
			WHERE user_id = $2 AND org_id = $3 AND deleted_at IS NULL
		`
		_, err = session.ExecCtx(ctx, removeQuery, currentUser.UserId, req.UserId, req.OrgId)
		if err != nil {
			return fmt.Errorf(config.ErrMsgOrganizationMemberRemove)
		}
		...
		if l.svcCtx.Config.SkylarkSyncEnabled && l.svcCtx.AdminSyncClient != nil {
			...
			syncResp, syncErr := l.svcCtx.AdminSyncClient.RemoveMember(syncCtx, &pb.RemoveMemberReq{
				TenantId:      tenantId,
				LocalOrgId:    req.OrgId,
			...
			if syncErr != nil {
				logx.WithContext(ctx).WithFields(
					...
					logx.Field("pod", l.svcCtx.PodName),
					logx.Field("module", "organization"),
					logx.Field("operation", "sync_remove_member_from_skylark"),
					logx.Field("status", "failed"),
					logx.Field("org_id", req.OrgId),
					logx.Field("user_id", req.UserId),
					logx.Field("error", syncErr.Error()),
				).Error("同步移除成员从 Skylark 失败，回滚事务")
				return fmt.Errorf("从 Skylark 移除成员失败: %v", syncErr)
			}
			...
			if !syncResp.Success {
				...
				return fmt.Errorf("从 Skylark 移除成员失败: %s", syncResp.Message)
			}
	...
	return &types.RemoveOrganizationMemberResponse{
		BaseResponse: types.BaseResponse{Code: 0, Msg: "移除成功"},
	}, nil
}
